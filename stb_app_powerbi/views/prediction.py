"""Vue Prédiction — prévision du montant total des transactions (production).

Holt-Winters (tendance amortie) et SARIMA(1,1,0)(0,1,1)[12], tous deux
appliqués sur le logarithme du montant (croissance multiplicative), avec un
...
render_header("Prévision des transactions")
...
if os.path.exists(default_path):
    ...
    uploaded_xlsx = st.file_uploader(
        "Déposez le fichier Excel des transactions", type=["xlsx"], key="forecast_xlsx_uploader"
    )
...
if source is not None:
    ...
    if resultats is not None:
        ...
        # ---------------------------------------------------------------
            ...
            st.dataframe(resultats["prev_hw"].round(0).rename("montant_prevu"), use_container_width=True)
        ...
        # ---------------------------------------------------------------
            ...
            st.dataframe(resultats["prev_sarima"].round(0).rename("montant_prevu"), use_container_width=True)
        ...
        st.download_button(
            ...
            buffer.getvalue(),
            file_name="previsions_transactions.xlsx",
